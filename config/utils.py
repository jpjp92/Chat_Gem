...
# youtube_transcript_api에 타임아웃 및 재시도 설정
                    ...
                    for entry in subtitle:
                        if isinstance(entry, dict):
                            if 'lines' in entry:
                                ...
                                logger.warning(f"Unexpected subtitle format: {entry!r}")
                    if full_text:
                        logger.info(f"yt-dlp 자막 추출 성공 (언어: {lang})")
                        return {'success': True, 'language': lang, 'text': full_text[:15000]}
                except Exception as e:
...
def get_youtube_info_fallback(video_id: str) -> Dict:
    ...
    try:
        ...
        logger.error(f"Video {video_id}: 대체 정보 추출 오류: {str(e)}")
        return {'success': False, 'error': f'Video {video_id}: 대체 정보 추출 오류: {str(e)}'}
