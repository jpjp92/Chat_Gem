        ...
        for session in st.session_state.chat_sessions:
            if session["id"] == st.session_state.current_session_id:
                ...
                for msg in st.session_state.messages:
                    ...
                    session["messages"].append(msg_copy)
    ...
    for session in st.session_state.chat_sessions:
        if session["id"] == session_id:
            ...
            for msg in session["messages"]:
                ...
                if "images" in msg_copy and msg_copy["images"]:
                    try:
                        msg_copy["images"] = [base64.b64decode(img.encode('utf-8')) for img in msg_copy["images"]]
                    except:
                        msg_copy["images"] = []
                st.session_state.messages.append(msg_copy)
            st.session_state.uploaded_images = []
            break
...
def is_image_analysis_request(query, has_images):
    """이미지 분석 요청인지 확인 - 더 정확한 판단"""
    if not has_images:
        return False
    exclude_keywords = ['유튜브', '웹사이트', 'pdf', '링크', 'url']
    if any(keyword in query.lower() for keyword in exclude_keywords):
        return False
    return True
...
def show_chat_dashboard():
    ...
    with chat_container:
        if "selected_message" in st.session_state:
            display_chat_message(st.session_state.selected_message)
        ...
        if get_usage_count() >= 100:
            ...
            if st.session_state.uploaded_images:
                for img_file in st.session_state.uploaded_images:
                    ...
                    img_file.seek(0)
                    processed_img = process_image_for_gemini(img_file)
            ...
            with st.status("🤖 요청을 처리하는 중...", expanded=True) as status:
                if is_youtube_request:
                    ...
                    try:
                        response = analyze_image_with_gemini_multiturn(processed_images, user_input, chat_session, detected_lang)
